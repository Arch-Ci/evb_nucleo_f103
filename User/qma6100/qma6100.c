...
#else
		#if defined(QST_USE_SW_I2C)
		ret_i2c =  (qs32)qst_sw_readreg(g_qma6100.slave<<1, reg_add, buf, num);
		#else
		ret_i2c =  (qs32)I2C_BufferRead(g_qma6100.slave<<1, reg_add, buf, (qu16)num);
		#endif
#endif
		if(ret_i2c == 1)
			return QMA6100_SUCCESS;
...
qs32 qma6100_read_acc_xyz(float *accData)
{
	...
	if(ret == QMA6100_SUCCESS)
	{
		tmpData[g_qma6100.cvt.map[0]] = g_qma6100.cvt.sign[0]*rawData[0];
		tmpData[g_qma6100.cvt.map[1]] = g_qma6100.cvt.sign[1]*rawData[1];
		tmpData[g_qma6100.cvt.map[2]] = g_qma6100.cvt.sign[2]*rawData[2];

		accData[0] = (float)(tmpData[0]*9.807f)/(g_qma6100.lsb_1g);			//GRAVITY_EARTH_1000
		accData[1] = (float)(tmpData[1]*9.807f)/(g_qma6100.lsb_1g);
		accData[2] = (float)(tmpData[2]*9.807f)/(g_qma6100.lsb_1g);
	}
