...
struct qmcX983_data
{
	...
	uint8_t			layout;
	//kal_int16			lsb_1g;	
	int16_t			xy_sensitivity;
	int16_t			z_sensitivity;
	int32_t			OTP_Kx;
	int32_t			OTP_Ky;
...
/**
  ...
  * @retval  
  */
uint8_t qmcX983_WriteReg(uint8_t reg_add,uint8_t reg_dat)
{
	uint8_t ret = 0;
	uint32_t retry = 0;
	...
	while((!ret) && (retry++ < 5))
	{
		...
		ret = qst_sw_writereg(QMCX983_SLAVE_ADDRESS, reg_add, reg_dat);
#else
		ret = I2C_ByteWrite(QMCX983_SLAVE_ADDRESS, reg_add, reg_dat); 
#endif
	}
...
/**
  ...
  * @retval  
  */
uint8_t qmcX983_ReadData(uint8_t reg_add,unsigned char* Read,uint8_t num)
{
	uint8_t ret = 0;
	uint32_t retry = 0;
...
/**
  ...
  * @retval  正常返回1，异常返回0
  */
static uint8_t qmcX983_device_check(void)
{
