...
unsigned char qmi8610_write_reg(unsigned char reg, unsigned char value)
{
	...
	while((!ret) && (retry++ < 5))
	{
		...
		ret = qst_sw_writereg(qmi8610_slave<<1, reg, value);
#else
		ret = I2C_ByteWrite(qmi8610_slave<<1, reg,value);
#endif
	}
	return ret;
}
...
#elif defined(QST_USE_SW_I2C)
		ret = qst_sw_writeregs(qmi8610_slave<<1, reg, value, len);
#else
		ret = I2C_BufferRead(qmi8610_slave<<1, reg, value, len);
#endif
	}
	return ret;
}
...
#elif defined(QST_USE_SW_I2C)
		ret = qst_sw_readreg(qmi8610_slave<<1, reg, buf, len);
#else
		ret = I2C_BufferRead(qmi8610_slave<<1, reg, buf,  len);
#endif
	}
	return ret;
}

void qmi8610_delay(int ms)
{
	int i,j;
	for(i=0;i<ms;i++)
	{
