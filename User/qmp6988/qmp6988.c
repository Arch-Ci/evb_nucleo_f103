...
void qmp6988_calc_pressure(qmp6988_data *qmp6988, float *press, float *tempearture)
{
	uint8_t err = 0;
//	uint8_t retry_count = 0;
	...
	pre = b00+bt1*Tr+bp1*P_raw+b11*Tr*P_raw+bt2*Tr*Tr+bp2*P_raw*P_raw+b12*P_raw*Tr*Tr+b21*P_raw*P_raw*Tr+bp3*P_raw*P_raw*P_raw;
#endif	
	...
	{
		float altitude;
		qmp6988_press_filter(press);
		altitude = (pow((101325/(*press)),1/5.257)-1)*(qmp6988->temperature+273.15)/0.0065;
	}
#endif
}
