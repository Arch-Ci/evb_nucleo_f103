...
void spi_struct_config(int mode)
{
	...
	SPI_InitStructure.SPI_NSS = SPI_NSS_Soft;
	SPI_InitStructure.SPI_BaudRatePrescaler = SPI_BaudRatePrescaler_16;	//SPI_BaudRatePrescaler_128;		//SPI_BaudRatePrescaler_256;
	SPI_InitStructure.SPI_FirstBit = SPI_FirstBit_MSB;
	SPI_InitStructure.SPI_CRCPolynomial = 7;
	SPI_Init(QST_SPIx , &SPI_InitStructure);
}
...
#define SPI_CS_HIGH				GPIO_SetBits(QST_SPI_CS_PORT, QST_SPI_CS_PIN) // CS----PB9
#define SPI_CS_LOW				GPIO_ResetBits(QST_SPI_CS_PORT, QST_SPI_CS_PIN)
...
u8 qst_acc_spi_write(u8 addr,u8 data)
{
#if defined(USE_SW_SPI)
	SPI_CS_LOW;
	spi_delay(1);
	spi_sw_write_data(addr&0x7f);
	spi_sw_write_data(data);
	spi_delay(1);
	SPI_CS_HIGH;
#else
	SPI_QST_CS_LOW();//SPI_CS_LOW;
...
u8 qst_acc_spi_read(u8 addr, u8* buff, u16 len)
{
	u16 r_data;
	...
	SPI_CS_LOW;//SPI_CS_LOW;
	spi_delay(1);
	spi_sw_write_data(0x80|addr);	// len low
	for(r_data=0;r_data<len;r_data++)
	{
		buff[r_data] = spi_sw_write_data(0x00);
	}	
	...
	SPI_QST_CS_LOW();//SPI_CS_LOW;
	spi_delay(1);
	Spi_SendByte(addr|0x80);	// addr low	
	for(r_data=0;r_data<len;r_data++)
	{	
		//buff[r_data] = SPI_I2S_ReceiveData(QST_SPIx);
		buff[r_data] = Spi_SendByte(0x00);
...
uint8_t qst_fisimu_spi_write(uint8_t Addr, uint8_t Data)
{
	SPI_QST_CS_LOW();
	spi_delay(1);
...
uint8_t qst_fisimu_spi_write_bytes(uint8_t Addr, uint8_t* Data, uint8_t Len)
{
	uint8_t i;

	for(i=0; i<Len; i++)
	{
		qst_fisimu_spi_write(Addr+i, Data[i]);
	}
...
uint8_t qst_8610_spi_read(uint8_t Addr, uint8_t *pData, uint16_t Length)
{
	uint8_t i = 0;
...
uint8_t qst_8658_spi_read(uint8_t Addr, uint8_t *pData, uint16_t Length)
{
	uint8_t i = 0;
